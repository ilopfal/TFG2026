...
// Pines para los 3 Sensores de Presión Reales
...
void setup() {
  ...
  // Activamos los pines 5 y 6 de la IMU
  Wire.begin(5, 6); 
  ...
  Serial.print("[CALIBRACION] P1 Zero Offset: "); Serial.print(offsetP1);
...
void loop() {
  if (dispositivoConectado) {
    ...
    pPresionChar->notify(); // -> AÑADIDO: Envía el dato de presión inmediatamente
    
    pImuChar->setValue(str_imu.c_str());
    pImuChar->notify(); // -> AÑADIDO: Envía el dato de movimiento inmediatamente
  }
  delay(50);
}
